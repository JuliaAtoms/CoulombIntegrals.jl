...
@testset "Accuracy tests" begin
    ...
    @testset "Yᵏ convergence rates" begin
        @testset "$Basis" for (Basis,Rf,Ns,errors) in [
            fedvr_inputs(range(1,stop=5,length=60),
                         [(6, "local", 2e-5, 7.0)]),
            ...
            @test test_convergence_rates(
                ...
                orbital_modes, coulomb_modes, Zs,
                sort(collect(keys(exact_Yᵏs)))) do N, orbital_mode, coulomb_mode, Z, ((n,ℓ), (n′,ℓ′), k)
                    (n′ > n || n′ == n && ℓ′ > ℓ) && return nothing
                    Yᵏ_error(Rf(rₘₐₓ/Z, N, Z)...,
                             k, n, ℓ, n′, ℓ′, Z,
    ...
    @testset "Fᵏ convergence rates" begin
        @testset "$Basis" for (Basis,Rf,Ns,errors) in [
            fedvr_inputs(range(2,stop=6,length=60),
                         [(11,"δF",2e-6,6.0),
                          (13,"δF′",2e-6,6.0)]),
            ...
            @test test_convergence_rates(
                ...
                ["N", "ρ", "a", "b", "c", "d", "k", "Exact", "Exact",
                 "F", "δ", "F′", "δ′", "F-F′", "Time"],
                orbital_modes, coulomb_modes, Zs, collect(keys(exact_Fᵏs))) do N, orbital_mode, coulomb_mode, Z, ((n,ℓ),(n′,ℓ′),k)
                    Fᵏ_error(Rf(rₘₐₓ/Z, N, Z)...,
                             k, n, ℓ, n′, ℓ′,
    ...
    @testset "Gᵏ convergence rates" begin
        @testset "$Basis" for (Basis,Rf,Ns,errors) in [
            ...
            @test test_convergence_rates(
                ...
                ["N", "ρ", "a", "b", "c", "d", "k", "Exact", "Exact",
                 "G", "δ", "Time"],
                orbital_modes, coulomb_modes, Zs, collect(keys(exact_Gᵏs))) do N, orbital_mode, coulomb_mode, Z, ((n,ℓ),(n′,ℓ′),k)
                    Gᵏ_error(Rf(rₘₐₓ/Z, N, Z)...,
                             k, n, ℓ, n′, ℓ′,
                             Z, orbital_mode, coulomb_mode)
                end
        end
